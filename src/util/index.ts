...
export class Util {
    ...
    public static isTextureAsset(name: string): boolean {
        const extensions = ['.ktx', '.ktx2', '.png', '.jpg', '.jpeg'];
    ...
    /**
     * For a given x,y for a scene, return the point generated by creating a ray from the given position and the
     * active camera's position. If there is no valid point (valid meaning a point on the surface of the mesh),
     * return null
     * @param xPosition - number, x coordinate to use
     * @param yPosition - number, y coordinate to use
     * @param scene - BabylonJS scene
     * @returns Nullable<Vector3>, either a valid point on the mesh or null
     */
    public static clickedPointOnMesh(xPosition: number, yPosition: number, scene: Scene): Nullable<Vector3> {
        if (scene != null && scene.activeCamera != null) {
            const ray = scene.createPickingRay(xPosition, yPosition, Matrix.Identity(), scene.activeCamera);
            const hit = scene.pickWithRay(ray);
            if (hit == null || hit.pickedMesh == null || hit.pickedPoint == null) {
                return null;
            }
    ...
    /**
     * Returns a list of (camera position, upvector) for the current active camera
     * @param scene - BabylonJS scene
     */
    public static cameraPositionAndView(scene: Scene): Array<Nullable<Vector3>> {
        const positionList = [];
        if (scene.activeCamera != null) {
            positionList[0] = scene.activeCamera.globalPosition;
            positionList[1] = scene.activeCamera.upVector;
        }
        return positionList;
    }

    /**
     * Gets mesh size in cm (width, height, depth)
     *
     * @param assetMesh the mesh to measure
     * @returns the mesh size in cm (width, height, depth)
     */
    public static measureMeshSizeInCentimeters = (assetMesh: AbstractMesh): CartesianMeasurement => {
        const boundingVectors = assetMesh.getHierarchyBoundingVectors();
        return {
            width: (boundingVectors.max.x - boundingVectors.min.x) * 100,
            height: (boundingVectors.max.y - boundingVectors.min.y) * 100,
            depth: (boundingVectors.max.z - boundingVectors.min.z) * 100,
        };
    };

    /**
     * Gets the max asset measurement length (width, height, depth)
     *
     * @param assetMeasurements the asset measurements (width, height, depth)
     * @returns the max asset measurement length
     */
    public static getMaxAssetMeasurementLength = (assetMeasurements: CartesianMeasurement): number => {
        return Math.max(assetMeasurements.depth, assetMeasurements.width, assetMeasurements.height);
    };
